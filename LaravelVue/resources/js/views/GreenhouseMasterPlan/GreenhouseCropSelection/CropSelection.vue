...
export default {
    ...
    methods: {
        async getJson(){    //取的資料庫資料，將資料轉成JSON
        ...
        },createCrop: async function(){     // 新增作物資料 函式呼叫
            ...
            // 判斷 要編輯的資訊是否為此專家所建立的
            if(this.CropData.Expert == 'System'){
                window.alert("此為系統參數，不可修改!!");
                return;
            }else if(this.$auth.user().id != this.CropData.Expert){
                window.alert(` ${this.CropData.VegetableTypes} 的參數是由專家${this.CropData.Expert}，不是您所建立，因此無法修改!`);
                return;
            }
