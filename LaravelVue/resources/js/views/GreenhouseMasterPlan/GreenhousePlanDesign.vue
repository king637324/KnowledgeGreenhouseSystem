                                                                ...
                                                                <v-col>
                                                                    <table style="border:1px solid black; " border='1'>
                                                                        ...
                                                                        <tr align="center">
                                                                            <td>工時分析</td>
                                                                            <td>工作日</td>
                                                                            <td colspan="3">{{400*Math.round((1+(area*0.2/1000))*100)/100}}</td>
                                                                        </tr>
                                                                    ...
                                                                    <div class="p-2 bd-highlight">
                                                                        <table style="border:1px solid black; font-size: 1.7vmin" border='1'>
                                                                            ...
                                                                            <tr align="center">
                                                                                <td>工時分析</td>
                                                                                <td>工作日</td>
                                                                                <td colspan="3">{{Math.round(20+(Math.round(SimpleJobDifficultyAdd*40*100)/100)*Math.round((1+Math.round(area*0.2/1000*100)/100)*100)/100*100)/100}}</td>
                                                                            </tr>
                                                                        </table>
                                                                    </div>
                                                                </v-col>
                                                                ...
                                                                <v-col>
                                                                    <table style="border:1px solid black; font-size: 1.7vmin" border='1'>
                                                                        ...
                                                                        <tr align="center">
                                                                            <td>工時分析</td>
                                                                            <td>工作日</td>
                                                                            <td colspan="3">{{600*Math.round((1+(area*0.2/1000))*100)/100}}</td>
                                                                        </tr>
                                                                ...
                                                                <v-col>
                                                                    <table style="border:1px solid black; font-size: 1.7vmin" border='1'>
                                                                        ...
                                                                        <tr align="center">
                                                                            <td>工時分析</td>
                                                                            <td>工作日</td>
                                                                            <td colspan="3">{{Math.round(30+(Math.round(RobustJobDifficultyAdd*60*100)/100)*Math.round((1+Math.round(area*0.2/1000*100)/100)*100)/100*100)/100}}</td>
                                                                        </tr>
                                                                    </table>
                                                                </v-col>
                                                            </v-row>
                                ...
                                <v-row v-if="overplanArray[0].userclass === '專家'">
                                    <b-card header-tag="header" header-text-variant="white" header-bg-variant="info">
                                        ...
                                        <b-card-text>
                                            <h5>
                                                請選擇欲更改之溫室型態：
                                                <v-radio-group row v-model="system_change"> 
                                                    <v-radio label="簡易溫室" id="簡易溫室" value="簡易溫室"></v-radio>
                                                    <v-radio label="強固溫室" id="強固溫室" value="強固溫室"></v-radio>
                                                </v-radio-group>
                                            </h5>
  ...
  export default {
    ...
    methods: {
        ...
        updateSimpleRadio: async function(greentype,simple){ //選擇管材時觸發此函式 更新資料庫內的資料
            ...
            if (this.SelectSimple_name.indexOf(greentype) === -1){
                this.SelectSimple_name.push(greentype)
                this.SelectSimple.push([greentype,simple])
            } else {
                this.SelectSimple[this.SelectSimple_name.indexOf(greentype)].splice(1,1,simple)
            }
            if (greentype === '溫室管材') {
                let formData = new FormData();
                formData.append('pipetype',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '圓頂形式') {
                let formData = new FormData();
                formData.append('rooftype',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '圓拱距') {
                let formData = new FormData();
                formData.append('circlespan',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '基礎') {
                let formData = new FormData();
                formData.append('base',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '跨距') {
                let formData = new FormData();
                formData.append('span',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '肩高') {
                let formData = new FormData();
                formData.append('shoulder',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '長度') {
                let formData = new FormData();
                formData.append('length',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '連續性') {
                let formData = new FormData();
                formData.append('continue',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            } else if (greentype === '披覆材料') {
                let formData = new FormData();
                formData.append('drape',simple.BuildItem);
                formData.append('_method','put');
                const response = await Design.UpdateDesign(this.now_user_design, formData);
            }
        ...
        updateRoofType: async function(){  //當選擇的項目已經九個時觸發 
            ...
            if (this.SelectSimple.length === 9 && this.greenhouseradio === '簡易溫室') {
                ...
                formData.append('pipetype',SelectSimple_copy[0][1].BuildItem);
                formData.append('rooftype',SelectSimple_copy[1][1].BuildItem);
                formData.append('circlespan',SelectSimple_copy[2][1].BuildItem);
                formData.append('base',SelectSimple_copy[3][1].BuildItem);
                formData.append('span',SelectSimple_copy[4][1].BuildItem);
                formData.append('shoulder',SelectSimple_copy[5][1].BuildItem);
                formData.append('length',SelectSimple_copy[6][1].BuildItem);
                formData.append('continue',SelectSimple_copy[7][1].BuildItem);
                formData.append('drape',SelectSimple_copy[8][1].BuildItem);
            }else if (this.SelectRobust.length === 9 && this.greenhouseradio === '強固溫室') {
                let SelectRobust_copy = new Array(9);
                for (var i = 0; i < this.SelectRobust.length; i++) {
                    if (this.SelectRobust[i][0] === '溫室型材'){
                        SelectRobust_copy[0] = this.SelectRobust[i]
                ...
                formData.append('housetype',this.greenhouseradio);
                formData.append('pipetype',SelectRobust_copy[0][1].BuildItem);
                formData.append('rooftype',SelectRobust_copy[1][1].BuildItem);
                formData.append('circlespan',SelectRobust_copy[2][1].BuildItem);
                formData.append('base',SelectRobust_copy[3][1].BuildItem);
                formData.append('span',SelectRobust_copy[4][1].BuildItem);
                formData.append('shoulder',SelectRobust_copy[5][1].BuildItem);
                formData.append('length',SelectRobust_copy[6][1].BuildItem);
                formData.append('continue',SelectRobust_copy[7][1].BuildItem);
                formData.append('drape',SelectRobust_copy[8][1].BuildItem);
            } else {
                formData.append('housetype',this.greenhouseradio);
            }
            formData.append('_method','put');
